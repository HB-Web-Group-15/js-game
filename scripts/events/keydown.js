...
document.addEventListener("keydown", (event) => {
  const { key } = event;
  const { selectedCell, gameActive, board } = gameState.getState();

  if (!gameActive || !board) return; // Ignore key presses if the game is not active or the board is not initialized
